...
impl<L: Language, N: AstNode<Language = L>> Iterator for AstSeparatedListElementsIterator<L, N> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let node = match slot {
            ...
            SyntaxSlot::Token(token) => panic!(
                "Malformed list, node expected but found token {:?} instead. You must add missing markers for missing elements.",
                token
            ),
            // Missing element
        ...
        let separator = match self.slots.next() {
            ...
            Some(SyntaxSlot::Node(node)) => panic!(
                "Malformed separated list, separator expected but found node {:?} instead. You must add missing markers for missing separators.",
                node
            ),
        };
...
mod tests {
    use crate::{
        AstNode, AstSeparatedElement, AstSeparatedList, SyntaxResult,
        raw_language::{
